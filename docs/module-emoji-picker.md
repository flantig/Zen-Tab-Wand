...
| `openEmojiPopover(rule, anchor, onChange)` | Opens a popover with a current-icon clear slot, search, and a 4x3 paged local emoji grid. Search matches category names, emoji characters, and bundled Unicode-style emoji names. Stores the selected emoji in `rule.icon` and calls `onChange()` after edits. |
