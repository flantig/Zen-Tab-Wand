...
| `openEmojiPopover(rule, anchor, onChange)` | Opens a popover with search and a 3x3 paged local emoji grid. Stores the selected emoji in `rule.icon` and calls `onChange()` after edits. |
