...
| `openEmojiPopover(rule, anchor, onChange)` | Opens a popover with search, a 3x3 paged local emoji grid, Clear, and Use text. Stores `rule.icon` as plain text and calls `onChange()` after edits. |
