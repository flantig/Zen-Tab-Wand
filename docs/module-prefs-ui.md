...
4. **alignSettingRows** — tags native preference rows so labels and controls share fixed columns. Checkbox controls stay first in DOM order for Sine's toggle code, then CSS grid places them in the shared control column.
...
6. **updateConditionalFields** — toggles the `.zao-pref-hidden` class on each AI-related row based on the engine pref. Engine `"off"` hides existing-behavior + new-group-behavior + all Ollama rows; `"local"` shows new-group behavior plus local batch size; `"ollama"` shows existing/new behavior plus Ollama host/model/warmup.
